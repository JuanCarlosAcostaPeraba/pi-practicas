...
w1:
	i2c_start();
	i2c_esc_byte(B11010000); // ult. byte escritura //CONTROL BYTE
	if (i2c_Rbit() != 0)
	{
		goto w1;
	}

	i2c_esc_byte(address); // ADDRESS LOW BYTE
	if (i2c_Rbit() != 0)
	{
		goto w1;
	}

	i2c_esc_byte(dato); // DATA BYTE //dato a escribir
	if (i2c_Rbit() != 0)
	{
		goto w1;
	}
...
w2:
	...
	i2c_esc_byte(B11010000); // Control byte
	if (i2c_Rbit() != 0)
	{
		goto w2;
	}
...
w3:
	...
	i2c_esc_byte(B11010001); // Control byte para leer (ult. byte a 1)
	if (i2c_Rbit() != 0)
	{
		goto w3;
	}

	byte data = i2c_lee_byte();
	i2c_Ebit1();
	i2c_stop();
	return data;
}
...
void teclado(int column)
{
	...
	{
	case 7:
		buffer = buffer + teclado_map[0][column];
		break;
	case 11:
		buffer = buffer + teclado_map[1][column];
		break;
	case 13:
		buffer = buffer + teclado_map[2][column];
		break;
	case 14:
		buffer = buffer + teclado_map[3][column];
		break;
